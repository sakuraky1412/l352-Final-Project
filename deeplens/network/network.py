...
class Network(object):
    ...
    def conv(self,
        ...
        with tf.variable_scope(name) as scope:
            kernel = self.make_var('weights', shape=[k_h, k_w, int(c_i) / group, c_o], weight_decay=weight_decay,
    ...
    def separable_conv(self,
        ...
        if dilation_rate > 1:
            assert s_h == 1 and s_w == 1
        separable_convolve = lambda i, k_depth, k_point: tf.nn.separable_conv2d(i, k_depth, k_point,
                                                                                strides=[1, s_h, s_w, 1],
                                                                                rate=[dilation_rate, dilation_rate],
                                                                                padding=padding, name=name)
        with tf.variable_scope(name) as scope:
            # kernel = self.make_var('weights', shape=[k_h, k_w, c_i / group, c_o])
            k_depth = self.make_var('kernel_depth', shape=[k_h, k_w, c_i, c_m], weight_decay=weight_decay,
                                    initializer=DEFAULT_INIT)
            k_point = self.make_var('kernel_point', shape=[1, 1, c_i * c_m, c_o], weight_decay=weight_decay,
                                    initializer=DEFAULT_INIT)
    ...
    def deconv(self,
        ...
        deconvolve = lambda i, k: tf.nn.conv2d_transpose(i, k, output_shape=[n_i, h_o, w_o, c_o / group],
    ...
    def batch_normalization(self, input, name, is_training=None, scale_offset=True, relu=False, lrelu=False, alpha=0.2,
        ...
        output = tf.layers.batch_normalization(input, training=is_training, name=name, center=scale_offset,
                                               scale=scale_offset, momentum=momentum, epsilon=epsilon, renorm=renorm)
